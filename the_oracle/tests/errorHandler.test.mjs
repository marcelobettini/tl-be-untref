// THE ORACLE — tests unitarios de errorHandler.
// Verifica las 8 rutas de mapeo y la garantía de no-filtrado.
...
function makeSdkError({ reason, status, statusText, message } = {}) {
  // Refleja la forma real del error del SDK. Los errorDetails son una CADENA JSON
  // en la ruta de fetch real (lo vimos en la prueba manual de humo de F3).
...
test("errorHandler refines 429 when SDK cause has RESOURCE_EXHAUSTED", async () => {
  const app = buildApp((_req, _res, next) => {
    ...
    next(
      new AppError(
        { statusCode: 502, safeMessage: "x", kind: "gemini" },
        sdkErr,
      ),
    );
  });
...
test("errorHandler maps unknown errors to 500 without leaking", async () => {
  const app = buildApp((_req, _res, next) => {
    next(
      new Error(
        "SECRET INTERNAL: db connection string was postgres://user:pass@host",
      ),
    );
  });
  const res = await request(app).get("/throw");
