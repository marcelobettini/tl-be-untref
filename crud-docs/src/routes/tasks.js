...
router.get("/:id", async (req, res) => {
  ...
  if (!task)
    return res
      .status(404)
      .json({ message: `No se encontró la tarea con id ${req.params.id}` });
  res.json(task);
});
...
router.post("/", async (req, res) => {
  ...
  if (!VALID_PRIORITIES.includes(priority)) {
    return res.status(400).json({
      message: `El campo priority debe ser uno de: ${VALID_PRIORITIES.join(", ")}`,
    });
  }
...
router.patch("/:id", async (req, res) => {
  ...
  if (!task)
    return res
      .status(404)
      .json({ message: `No se encontró la tarea con id ${id}` });
